<script lang="ts">
  ...
  async function registerKey(): Promise<void> {
    ...
    try {
      response = await nodeState.registerKey(walletState.address, pubKey, url);
    ...
    try {
      await nodeState.loginKeyChallange(walletState.address, challange, url);
    } catch (e) {
  ...
  async function loginKey(): Promise<void> {
    ...
    let response: Response;
    try {
      response = await nodeState.getKeyChallange(walletState.address, url);
      if (response.status !== 200) {
        throw response.status;
      }
    } catch (e) {
    ...
    <DialogContent class="w-full max-w-[500px]">
      ...
      <div class="flex w-full flex-col">
        <Button
          ...
          onclick={() => walletState.connectWeb()}>Connect Wallet</Button
        >
        <div>
      ...
      <div class="flex items-center w-full">
        <Button onclick={() => registerKey()} class="m-3 w-full"
          >Register</Button
        >
        <Button onclick={() => loginKey()} class="m-3 w-full">Login</Button>
      </div>
      <div class="flex items-center w-full">
        <span>
          {#if errorMessage}
      ...
      <DialogFooter>
        <Button class="m-3" variant="secondary" onclick={() => (open = false)}
          >Close</Button
        >
      </DialogFooter>
    </DialogContent>
